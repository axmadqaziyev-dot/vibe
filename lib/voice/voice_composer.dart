...
class _VoiceComposerState extends State<VoiceComposer>
  ...
  void initState() {
    ...
    // Start from the user's microphone action.
    unawaited(start());
  }
  ...
  Future<void> start() async {
    ...
    try {
      ...
      ticker = Timer.periodic(const Duration(milliseconds: 200), (_) {
        ...
        if (elapsed >= maxVoiceSeconds * 1000 && !busy) unawaited(stop());
      });
    } catch (e) {
      if (mounted)
        setState(() {
  ...
  Future<void> stop() async {
    ...
    try {
      ...
      if (path == null) throw StateError('Səs faylı yaranmadı.');
  ...
  void dispose() {
    ...
    unawaited(() async {
      try {
        ...
        if (file != null) await releaseAudio(file);
      } catch (_) {}
    }());
    super.dispose();
  }
                ...
                child: draft != null
                    ...
                    : Text(
                        recording
                            ...
                            : 'Səsli mesaj',
                        style: TextStyle(color: recording ? Colors.red : null),
                      ),
              ),
              if (recording)
                IconButton.filled(
